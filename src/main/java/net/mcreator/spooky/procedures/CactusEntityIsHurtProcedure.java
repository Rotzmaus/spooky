...
public class CactusEntityIsHurtProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			_entity.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 200, 2, (false), (false)));
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof LivingEntity _livEnt ? _livEnt.hasEffect(SpookyModMobEffects.PLAYER) : false) {
					{
						double _setval = (entityiterator.getCapability(SpookyModVariables.PLAYER_VARIABLES_CAPABILITY, null)
								.orElse(new SpookyModVariables.PlayerVariables())).effectiveHealthPoints - 2;
						entityiterator.getCapability(SpookyModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
							capability.effectiveHealthPoints = _setval;
							capability.syncPlayerVariables(entityiterator);
						});
					}
					entityiterator.hurt(DamageSource.CACTUS, 1);
